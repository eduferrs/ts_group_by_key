 ...
 * O TS irá impedir que se tente agrupar por uma chave que não exista (no tipo genérico T)
 * 
 * E a função irá retornar um objeto genérico GroupsMap
 * Onde os VALORES da chave passada como argumento (key, uma chave existente nos obj de 
 * items) viram chaves no GroupsMap retornado
 */
export function groupByKey<T, K extends keyof T>(
  ...
  for (const item of items) {
    // Pega o valor da propriedade e convertendo em string para usar como chave do objeto GroupsMap
    // Ex.: se items é uma lista de objetos no formato { id: 1, color: 'red', country: 'Ukraine' }
